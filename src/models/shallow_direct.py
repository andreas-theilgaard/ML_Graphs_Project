...
@torch.no_grad()
def test(model, x, split_edge, evaluator, batch_size):
    model.eval()
    ...
    for perm in DataLoader(range(pos_train_edge.size(0)), batch_size):
        ...
        pos_train_preds += [
            torch.sigmoid(model(edge[0], edge[1])).cpu()
        ]  # [model(x[edge[0]], x[edge[1]]).squeeze().cpu()]
...
class ShallowTrainer:
    ...
    def metrics(self, pred, label, type="accuracy", logits_in=True):
        if type == "accuracy":
            yhat = torch.sigmoid(pred) if logits_in else pred
            yhat = (pred > 0.5).float()
    ...
    def fit(self, dataset, seeds):
        ...
        for epoch in prog_bar:
            ...
            if epoch % 10 == 0:
                results = test(
                    model=model,
                    x=model.embeddings.weight.data.cpu(),
                    split_edge=split_edge,
                    evaluator=Evaluator(name=self.config.dataset.dataset_name),
                    batch_size=self.training_args.batch_size,
                )
                print(results["hits@10"])
                print(results["hits@50"])
