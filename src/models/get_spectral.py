...
def get_spectral(K, task, dataset_name, edge_split=None, config=None):
    ...
    if "save_to_folder" in config:
        ...
        additional_save_path = f"{config.save_to_folder}/{config.dataset.task}/{config.dataset.dataset_name}/{config.dataset.DIM}/Spectral"
        create_path(f"{additional_save_path}")
        torch.save(eigen_vectors, additional_save_path + "/spectral.pth")
