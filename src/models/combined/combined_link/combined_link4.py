    ...
    for perm in DataLoader(range(pos_edge_index.size(1)), batch_size, shuffle=True):
        ...
        if config.dataset.dataset_name == "ogbl-citation2":
            ...
            if training_args.direct:
                # Negative edges
                neg_out_deep = deep(data_deep.x, data_deep.adj_t, edge[0], dst_neg)
            else:
                # Negative edges
                neg_out_deep = decode(
                    W=W,
...
def fit_combined4_link(config, dataset, training_args, Logger, log, seeds, save_path):
        ...
        for epoch in prog_bar:
            ...
            for perm in DataLoader(range(pos_edge_index.size(1)), training_args.batch_size, shuffle=True):
                ...
                if config.dataset.dataset_name == "ogbl-citation2":
                    ...
                    if training_args.direct:
                        neg_out_deep = deep(
                            data_deep.x, data_deep.adj_t, neg_edge_index[0], neg_edge_index[1]
                        )
                    else:
                        neg_out_deep = decode(
                            W=W,
    ...
    Logger.end_run()
    Logger.save_results(save_path + "/combined_comb4_results.json")
