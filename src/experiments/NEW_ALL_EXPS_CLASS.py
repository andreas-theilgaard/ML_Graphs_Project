...
if __name__ == "__main__":
    ...
    for dataset in ["Cora-class", "Twitch-class", "Pubmed-class", "Flickr-class", "CiteSeer-class"]:
        for DIM in [2, 8, 16]:
            ...
            for shallow_embedding in shallow_embeddings:
                subprocess.call(
                    f"{CONFIG_SETUP} model_type='GNN_DIRECT' runs=1 dataset.DIM={DIM} dataset.GNN_DIRECT.extra_info={Shallow_path+'/'+shallow_embedding} dataset.GNN_DIRECT.model='GraphSage'",
                    shell=True,
                )
                subprocess.call(
                    f"{CONFIG_SETUP} model_type='GNN_DIRECT' runs=1 dataset.DIM={DIM} dataset.GNN_DIRECT.extra_info={Shallow_path+'/'+shallow_embedding} dataset.GNN_DIRECT.model='GCN'",
                    shell=True,
                )
            # Run GNN Direct with spectral
            subprocess.call(
                f"{CONFIG_SETUP} model_type='GNN_DIRECT' runs={args.runs} dataset.DIM={DIM} dataset.GNN_DIRECT.use_spectral=True dataset.GNN_DIRECT.model='GCN'",
                shell=True,
            )
            subprocess.call(
                f"{CONFIG_SETUP} model_type='GNN_DIRECT' runs={args.runs} dataset.DIM={DIM} dataset.GNN_DIRECT.use_spectral=True dataset.GNN_DIRECT.model='GraphSage'",
                shell=True,
            )
            ...
            # Run combined method
            subprocess.call(
                f"{CONFIG_SETUP} model_type='combined' runs={args.runs} dataset.DIM={DIM} dataset.combined.type='comb1' dataset.combined.comb1.training.MLP_HIDDEN={DIMENSIONS[DIM]}",
                shell=True,
            )
            subprocess.call(
                f"{CONFIG_SETUP} model_type='combined' runs={args.runs} dataset.DIM={DIM} dataset.combined.type='comb2' dataset.combined.comb2.training.MLP_HIDDEN={DIMENSIONS[DIM]}",
                shell=True,
            )
